...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      // Stop loop entirely when paused (off-screen or tab hidden)
      if (!shouldRenderRef.current || !materialRef.current || !rendererRef.current || !sceneRef.current || !cameraRef.current) {
        rafRef.current = null
        return
      }
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        shouldRenderRef.current = visible
